...
fn heightMapTracing(ori: vec3f, dir: vec3f) -> (f32, vec3f) {
    var tm: f32 = 0.0;
    var tx: f32 = 1000.0;
    var p: vec3f;
    ...
    return (mix(tm, tx, hm / (hm - hx)), p);
}

fn calculate_dir(uv: vec2f) -> vec3f <f32>{
...
const vec3<f32> pos = vec3<f32>(3.0);
